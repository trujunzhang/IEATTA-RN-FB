...
class BGService {
  ...
  */
  loadTestGeofences(route, config, callback) {
    callback = callback || function(){};
    let data = this.getCityDriveData();
    let geofences = [];
  ...
  */
  ...
  computeOffsetCoordinate(coordinate, distance, heading) {
    ...
    let lat1 = this.toRad(coordinate.latitude), lon1 = this.toRad(coordinate.longitude);
    let lat2 = Math.asin(Math.sin(lat1) * Math.cos(distance) +
                        Math.cos(lat1) * Math.sin(distance) * Math.cos(heading));

    let lon2 = lon1 + Math.atan2(Math.sin(heading) * Math.sin(distance) *
                                Math.cos(lat1), 
